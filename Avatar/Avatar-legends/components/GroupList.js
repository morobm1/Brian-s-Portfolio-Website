function GroupList() {
    ...
    const groupbending = [ 
        {
            title:"Group placeholder - Attend to Commands",
            approach:"Evade & Observe", 
            desc:"A leading voice in the group takes a moment to organize it effectively. The group clears Impaired, becomes Inspred, and inflects an additional 1-fatigue on all attacks made next exchange."
        },
        {
            title:"Coordination",
            approach:"Evade & Observe", 
            desc:"The group sets itself up to lauch a concerted, skilled attack upon its targets next exchange. The group becomes Prepared and Favored and may clear 2-fatigue or a conditions(in addition to the normal  1-fatigue for evade and observe)."
        },
        {
            title:"Draw Foe",
            approach:"Evade & Observe", 
    ...
    const groupList = groupbending.map(group => 
        <div>
            <h1>{group.title}</h1>
            <h4>{group.approach}</h4>
            <p>{group.desc}</p>
