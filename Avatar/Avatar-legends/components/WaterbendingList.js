...
const waterbending = [ 
    {
        title:"Water placeholder -  Blood Twisting",
        approach:"Advance & Attack", 
        desc:"Use bloodbending to move and twist a foe's body in painful ways. You must be Empowered to use this technique. Inflict a condition on your foe. If they are already Impaired, Trapped, or Doomed, inflict an additional condition. If this is your first, second, or third time ever using this techinque, mark a condition."
    },
    {
        title:"Breath of Ice",
...
const waterList = waterbending.map(water => 
    <div>
        <h1>{water.title}</h1>
        <h4>{water.approach}</h4>
        <p>{water.desc}</p>
        

    </div>)

    return <div>{waterList}</div>
